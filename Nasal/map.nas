...
var setupCanvas = func {
	...
	mycanvas.addPlacement({"node": "mfi_page2_map", "texture": "MAPscreen.png"});
}
...
var scale_prop = "controls/instrumentation/mfis/scale";
var zooms      = [8, 9, 10, 11, 12, 13, 14];
var zoomLevels = [200, 150, 100, 75, 50, 25, 12.5];
var zoom_curr  = 0;
...
var setZoom = func(index) {
  index = int(index);
  if (index < 0) {
    index = 0;
  } elsif (index >= size(zooms)) {
    index = size(zooms) - 1;
  }
  zoom_curr = index;
  ...
  if (zoom_syncing == 0) {
    zoom_syncing = 1;
    setprop(scale_prop, zoom_curr);
    zoom_syncing = 0;
  }
}

var syncZoomFromScale = func {
  if (zoom_syncing == 1) {
    return;
  }
  var scale = getprop(scale_prop);
  if (scale == nil) {
    scale = 0;
  }
  var idx = int(scale);
...
var toggleScale = func(delta) {
  setZoom(zoom_curr + (delta == nil ? 1 : delta));
}
...
var makeUrl =
	...
	#string.compileTemplate('https://maps.wikimedia.org/{z}/{x}/{y}.png');
	#string.compileTemplate('http://otile1.mqcdn.com/tiles/1.0.0/map/{z}/{x}/{y}.png');
	#string.compileTemplate('https://tiles.wmflabs.org/hikebike/${z}/${x}/${y}.png');
var makePath =
...
var last_type = type;
var last_zoom = zoom;
var lastLiveMap = getprop("controls/instrumentation/mfis/live-map");
var lastDay   = TRUE;
...
var MM = {
	...
	setupCanvasSymbols: func {
		...
		me.filename = "Nasal/littleaircraftRed.svg";
		...
		me.svg_symbol = me.rootCenter.createChild("group");
		canvas.parsesvg(me.svg_symbol, me.filename);
		me.svg_symbol.setScale(0.05);
		me.svg_symbol.setTranslation(-24,-18);
		me.myVector = me.svg_symbol.getBoundingBox();
		me.svg_symbol.updateCenter();
		me.svg_symbol.set("z-index", 3);
			
		# own symbol
		# simple aircraft icon at current position/center of the map
    # me.filename = "Nasal/littleaircraftRed.svg";
    # me.svg_symbol = me.rootCentre.createChild("group");
    # canvas.parsesvg(me.svg_symbol, me.filename);
    # me.svg_symbol.setScale(0.05);

    # me.svg_symbol.setTranslation((width/2)-20,height/2-45);
    # me.myVector = me.svg_symbol.getBoundingBox();
    # me.svg_symbol.updateCenter();
    # me.svg_symbol.set("z-index", 3);

		me.hdg_line = me.rootCenter.createChild("path")
			# .moveTo(0, -15)
			# .lineTo(0, 16)
			# .moveTo(-5, 12)
			# .lineTo(5, 12)
...
#	    }
	...
	loop: func {
		...
		if (me.off == TRUE) {
			setprop("controls/instrumentation/mfis/brightness-mm", 0);
			settimer(func{me.loop();},0.5);
			return;
		} else {
			setprop("controls/instrumentation/mfis/brightness-mm", me.brightness);
...
#printf("centertile: %d,%d fraction %.2f,%.2f",me.center_tile_int[0],me.center_tile_int[1],me.center_tile_fraction_x,me.center_tile_fraction_y);
		...
		me.liveMap = getprop("controls/instrumentation/mfis/live-map");
					...
					(func {# generator function
					    ...
					    if( io.stat(img_path) == nil and me.liveMap == TRUE) { # image not found, save in $FG_HOME
					          ...
					          .fail(func (r) {#print('Failed to get image ' ~ img_path ~ ' ' ~ r.status ~ ': ' ~ r.reason);
					          				tile.set("src", "Aircraft/Yak-152/Models/Interior/Panel/Instruments/mfi/map/emptyTile.png");
					      					tile.update();
					      					});
					    	...
					    	tile.set("src", "Aircraft/Yak-152/Models/Interior/Panel/Instruments/mfi/map/noiseTile.png");
					      	tile.update();
					    }
					})();
		  		}
...
var init = func {
	...
	setupCanvas();
	syncZoomFromScale();
	setlistener(scale_prop, syncZoomFromScale, nil, 0);
	mm = MM.new();
	settimer(func {
		mm.loop();
