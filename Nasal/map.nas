...
var MAPCanvas = {
  ...
  new: func(placement) {
    ...
    # m.makeUrl  = string.compileTemplate('http://{server}.mqcdn.com/tiles/1.0.0/sat/{z}/{x}/{y}.jpg');
    # m.servers = ["otile1", "otile2", "otile3", "otile4"];

    # m.makeUrl  = string.compileTemplate('http://{server}.tile.osm.org/{z}/{x}/{y}.png');

    m.makeUrl  = string.compileTemplate('https://tiles.wmflabs.org/hikebike/${z}/${x}/${y}.png');
    m.servers = ["a", "b", "c"];
...
var MapCanvas = yak152.MAPCanvas.new({"node": "mfi_page2_map", "texture": "MAPscreen.png"});
